...
VIA_EMAIL, VIE_PHONE  = ("via_email", "via_phone")
...
class UserConfirmation(BaseModel):
    ...
    experation_time = models.DateTimeField(null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            if self.verify_type == VIA_EMAIL:
                self.experation_time = datetime.now() + timedelta(minutes=EMAIL_EXPIRE)
            else:
                self.experation_time = datetime.now() + timedelta(minutes=PHONE_EXPIRE)
